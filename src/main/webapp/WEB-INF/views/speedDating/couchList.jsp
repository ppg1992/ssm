...
function getBigBoardCaseInfo(tab1){
	...
	var wxaddresscouchList = ctx + "/speed/getBigCaseDataList";
	$('#loadingToast').show();
	$.ajax({
		type : "POST",
		...
		success : function(data) {
			...
			for(var i = 0 ; i < data.length ; i++){
				...
				str = "<li onclick='detail("+data[i].id+");'>";
				...
				str += '<p class="list-time'+timeColor+'">报案时间：'+getDateStr(data[i].entryTime)+'</p>';
...
function detail(id){
	window.location.href = ctx +"/speed/geBigtCaseDetail?id="+id;
}
