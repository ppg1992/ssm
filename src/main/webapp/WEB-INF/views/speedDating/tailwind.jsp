...
<div id="loadingSearch" class="weui_loading_toast" style="display:none;">
    ...
    <div class="weui_toast">
        ...
        <p class="weui_toast_content">查询价格中,请稍后</p>
    </div>
</div>
...
<div id="wrap">
	...
	<form id="wxreportcaseform" action="#" method="post">
		...
		<input type="hidden" id="caseMark" name="caseMark" value="2" />
		...
		<input type="hidden" id="predictTrailerKm" name="predictTrailerKm" value=""/>
    <div class="item">
	...
	<div class="content mtop4">
		...
		<div class="my-item">
			...
			<ul class="report-intro report-intro1 clearfix pb1">
				...
				<li class="report-list">
					<input class="fl input" id="location" name="rescueAddress" readonly value="" placeholder="请选择作业地点"/>
				</li>
		...
		<div class="my-item">
			...
			<ul class="report-intro report-intro1 clearfix pb1">
				...
				<li class="report-list">
					<input class="fl input destination"  id="desAddress" name="desAddress" readonly value="" placeholder="请选择目的地" />
				</li>
        ...
        <div class="item">
			...
			<div class="price-content clearfix">
				...
				<div class="right-price fr border-radius">
					<p class="price"><span>6折</span>起</p>
					<b class="price-flag border-radius">顺风车价格</b>
				</div>
			</div>
			<div class="read">
...
// 	console.log(locationArray+"---"+desAddressArray);
	dis = AMap.GeometryUtil.distance(locationArray,desAddressArray);
	dis = Math.round(dis / 1000) ;
	$("#predictTrailerKm").val(dis);
// 	console.log(dis);
	ctx = $('#ctx').val();
...
$(".wxSubmit").on('click',function(){
	...
	var newstr = Date.parse(reserveTimeEnd.split('-').join("/"));
	var nowFormatDate = getNowFormatDate(0);
	var newstr1 = Date.parse(nowFormatDate.split('-').join("/"));
	...
	if(!$("#customeIndex").val()){
    	...
    	return alert('公里数需大于30公里！');
    }else if((newstr - newstr1) < 2*60*60*1000){
		return alert('预约结束时间必须大于当前两小时！');
	}
	var isReturn = true;
